...
# As your starting point, use the enhanced version from class on September 6. Omit the part about tt_type (tautology, contingency, contradiction).
...
# It is not necessary to include the other functions from that assignment
# (De Morgan's Laws, Modus Ponens, Modus Tollens, and Hypothetical Syllogism).
def f0_false(p, q):
    return 0
...
def analyze_functions():
    functions = [f0_false, f1_nor, f2_converse_nonimpl, f3_not_p, f4_not_impl, f5_not_q, f6_xor, f7_nand, f8_and,
                 f9_xnor, f10_q, f11_imply, f12_p, f13_converse, f14_or, f15_True]
    data = []
    for func in functions:
        results = analyze_boolean_table(func)
        row = [func.__name__, func_body(func), str(results[6]), results[7], reverse_engineer(func)]
        data.append(row)
    headers = ["name", "functions", "binary", "decimal", "reversed"]
...
def half_adder():
    ...
    # c = b_and(x, y)
    data = [[x, y, b_and(b_or(x, y), b_not(b_and(x,y))), b_and(x, y)] for x in [1,0] for y in [0,1]]
    headers = ["x", "y", "s", "c"]
    title = "half-adder"
    alignments = ["r", "r", "r", "r"]
